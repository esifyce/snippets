...
final class CarouselViewController: UIViewController {
    ...
    var carouselIndicator: UICollectionView!
...
private extension CarouselViewController {
    private func setupCollectionView() {
      ...
      private func setupCarouselIndicator() {
        let layout = UICollectionViewFlowLayout()
        layout.scrollDirection = .horizontal
        layout.minimumInteritemSpacing = 10
        carouselIndicator = UICollectionView(frame: .zero, collectionViewLayout: layout)
        carouselIndicator.delegate = self
        carouselIndicator.dataSource = self
        carouselIndicator.register(CarouselIndicatorCollectionViewCell.self, forCellWithReuseIdentifier: "indicatorCell")
        view.addSubview(carouselIndicator)
      }
    
    func setConstraints() {
        collectionView.translatesAutoresizingMaskIntoConstraints = false
        carouselIndicator.translatesAutoresizingMaskIntoConstraints = false
        ...
        let carouselIndicatorConstraints = [
          carouselIndicator.topAnchor.constraint(equalTo: collectionView.bottomAnchor, constant: 20),
          carouselIndicator.leadingAnchor.constraint(equalTo: view.leadingAnchor),
          carouselIndicator.trailingAnchor.constraint(equalTo: view.trailingAnchor),
          carouselIndicator.heightAnchor.constraint(equalToConstant: 40)
        ]
        NSLayoutConstraint.activate(carouselIndicatorConstraints)
    }
}


extension CarouselViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
